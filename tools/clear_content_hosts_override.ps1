...
$statusLog = Join-Path $backupDir 'content-hosts-clear-status.log'
$errorLog = Join-Path $backupDir 'content-hosts-clear-error.log'
...
$backup = Join-Path $backupDir "hosts.content.clear.backup-$timestamp"
$tempHosts = Join-Path $backupDir "hosts.content.clear.tmp-$timestamp"
try {
    ...
    "Removed config/content/lobby hosts overrides" | Add-Content $statusLog -Encoding ASCII
