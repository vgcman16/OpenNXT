...
$pythonExe = "python"
$traceScript = Join-Path $PSScriptRoot "trace_rs2client_exception.py"
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $root ("data\\debug\\frida-exception\\rs2client-exception-{0}.jsonl" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
}
