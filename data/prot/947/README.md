...
- The Phase 1 driver now recovers the server registrar generically and falls back to
  `DumpCallRefsForTarget.java` for the current client registrar when the generic
  registrar scan does not surface it
- Runtime handler binding is now traced through `descriptor + 0x48` and into the real parser entrypoints
- Phase 3 family classification now emits confidence-scored candidate output and
  generated draft name files under `generated/phase3`
- Phase 4 verification preparation now emits field/runtime work queues under
  `generated/phase4`
...
Curated historical compare:

- checked-in labels file:
  `C:\Users\Demon\Documents\New project\OpenNXT\data\prot\946\curated-session-labels.json`
- operator guidance:
  - the labels file is the trusted representative set for curated historical
    compare; use it to pin a few stable, bad, and rebuild-fail windows instead
    of letting unlabeled outliers drive seed selection
  - update it when a session becomes a reliable new baseline or a recurring
    regression case
  - `advisoryReady=true` means the curated compare has enough labeled cohort
    coverage to be trusted for decision support; treat `low` confidence as
    informational only
  - curated compare is pipeline-ready advisory evidence, not a build-failing CI
    gate

Operator readiness surfaces:

- doctor:
  - `C:\Users\Demon\Documents\New project\OpenNXT\tools\run_946_tool_doctor.py`
  - validates prerequisite files, labels, world-log presence, Ghidra/decomp
    availability, and advisory artifact freshness in one report
- fixture pack:
  - `C:\Users\Demon\Documents\New project\OpenNXT\tools\run_946_fixture_pack.py`
  - extracts labeled world-log windows into small checked-in fixture bundles
    under `data/prot/946/fixtures`
- phase drivers:
  - `run_946_phase2.py`, `run_946_phase3.py`, `run_946_phase4.py`,
    `run_946_phase5.py` remain the low-level phase-entry scripts
  - treat them as internal pipeline phases first and standalone debugging entry
    points second
...
- current generated outputs:
  - `generated/phase2/serverParsers.json`
  - `generated/phase2/clientParsers.json`
  - `generated/phase2/handlerFamilies.json`
  - `generated/phase2/phase2-summary.md`

Phase 3 driver:

- `C:\Users\Demon\Documents\New project\OpenNXT\tools\run_946_phase3.py`
- current generated outputs:
  - `generated/phase3/nameCandidates.json`
  - `generated/phase3/clientAmbiguousClusters.json`
  - `generated/phase3/serverProtNames.generated.toml`
  - `generated/phase3/clientProtNames.generated.toml`
  - `generated/phase3/serverProtNames.uniqueCandidates.generated.toml`
  - `generated/phase3/clientProtNames.uniqueCandidates.generated.toml`
  - `generated/phase3/phase3-summary.md`
- current client prompt evidence now also includes dispatcher-selector hints for
  the ambiguous `RESUME_*` raw-string trio
- the generated Phase 3 outputs now also break those into explicit ambiguous
  client clusters so later passes can sort by dispatcher/selector instead of
  re-deriving the same grouping from the full candidate dump
- current family classification coverage:
...
- current generated outputs:
  - `generated/phase4/fieldGapAnalysis.json`
  - `generated/phase4/fieldRecoveryQueue.json`
  - `generated/phase4/runtimeVerificationQueue.json`
  - `generated/phase4/runtimeFocus.json`
  - `generated/phase4/parserFieldIndex.json`
  - `generated/phase4/parserFields/*.json`
  - `generated/phase4/manualFieldDraftOverrides.json`
  - `generated/phase4/phase4-summary.md`
- current role:
  - diff named `946` packets against matching `919` field files when available
  - turn Phase 3 output into a prioritized field-recovery queue
  - turn remaining family/exact-name evidence into a prioritized
    runtime-verification queue
  - run `ExportParserFields.java` sequentially against the top safe
    field-recovery targets
- current safety rails:
  - do-not-touch blacklist for `PLAYER_INFO`, `NPC_INFO`, `REBUILD_NORMAL`,
    `MAP_PROJANIM`, and `MAP_PROJANIM_HALFSQ`
  - zero-byte packets are excluded from auto field-file generation
  - the parser-field extractor emits unknown transform markers instead of
    guessing transformed byte/int types when the evidence is weak
  - explicit packet re-extraction now also works for already-promoted packets,
    so Phase 4 summaries stay in sync after new `*.txt` files land
  - reviewed manual draft overrides can be merged through
    `generated/phase4/manualFieldDraftOverrides.json` when a packet's semantics
    are proven by decompile review before the generic extractor catches up
- current extractor notes:
  - `ExportParserFields.java` currently uses decompiler-driven structural
    heuristics and safe unknown fallbacks; it is useful for draft recovery but
    is not yet a full Pcode data-flow proof pass
  - it now also recognizes direct scalar packet loads (`*(uint *)`,
    `*(ushort *)`, `*(char *)`) and follows the common byte-swap/rotate/negate
    transform patterns used by several `IF_*` parsers
  - it now also drops shadowed direct-scalar byte loads when a larger recovered
    field already consumes the same parser locals, which keeps mixed
    scalar-plus-composite `IF_*` drafts from exploding into every temporary byte
  - it now also recovers top-level call-argument short expressions separately
    from adjacent four-byte expressions, which fixed the `IF_SETRECOL` draft and
    exposed the merged transformed short inside `IF_SETRETEX`
...
- current first-pass draft results:
  - `IF_SETEVENTS` -> `mask intle`, `fromSlot ushortle`,
    `parent intv2`, `toSlot ushortle128`
  - `IF_SETTEXT` -> `string`, `intle`
  - `RESUME_P_COUNTDIALOG` -> sender-side `count long`
  - `IF_SETCOLOUR` -> `intv1`, `ushort128`
  - `IF_OPENSUB_ACTIVE_OBJ` -> `ushort`, `ushort`, `intv1`, `ubytec`, `intv1`
  - `IF_SETPLAYERHEAD` -> `intv1`
  - `IF_SETPLAYERMODEL_SELF` -> `intv2`
  - `IF_SETSCROLLPOS` -> `component intle`, `scrollPosition ushort`
  - `IF_SETRETEX` -> `ubytec`, `intv1`, `ushort128`, `ushortle`
  - `IF_SETRECOL` -> `ushortle`, `ushortle`, `ubytec`, `intle`
  - `OBJ_REVEAL` -> `count ushortle`, `id ushortle`, `packedCoord ubyte`,
    `playerIndex ushortle`
- current helper entry points for live verification:
  - `tools/trace_process_runtime.py`
  - `tools/trace-win64c-runtime.ps1`
...
Current reviewed high-confidence automation promotions:

- Server `6` -> `LOC_ANIM_SPECIFIC`
- Server `7` -> `IF_OPENSUB_ACTIVE_OBJ`
- Server `137` -> `IF_SETRETEX`
- Server `202` -> `IF_SETRECOL`
- Client `110` -> `WORLDLIST_FETCH`
...
- Phase 3 now carries those three opcodes as ambiguous exact-name candidates:
  `RESUME_P_STRINGDIALOG`, `RESUME_P_NAMEDIALOG`, and `RESUME_P_OBJDIALOG`
- the generated Phase 3 evidence now also records the shared dispatcher
  `FUN_14003b9e0` plus the current selector values observed in that prompt
  cluster:
  `67 -> 0x323`, `93 -> 0x7d7`, `24 -> 0x754`, `87 -> 0x203`
...
- `59` decompiles to `FUN_14019b3f0`, and that helper proved the `IF_SETEVENTS`
  semantic argument order directly: `parent`, `fromSlot`, `toSlot`, `mask`
- `137` now recovers cleanly as `ubytec`, `intv1`, `ushort128`, `ushortle`,
  while `202` recovers as `ushortle`, `ushortle`, `ubytec`, `intle`
